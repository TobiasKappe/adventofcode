...
fillVer walls bottom visited pos =
    ...
    then (False, visited)
         ...
         then (True, visited)
              ...
              then (blocked visited West pos && blocked visited East pos, visited)
...
fill walls = 
    let ys = S.map (\(Pos x y) -> y) walls
        top = S.findMin ys
        bottom = S.findMax ys
        filled = S.fromAscList $ M.keys $ snd $ fillVer walls bottom M.empty (Pos 500 0)
    in S.filter (\p -> depth p >= top) filled
