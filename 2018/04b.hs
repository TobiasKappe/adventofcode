...
mostSleepyMinute sleepMap =
    let minutesSlept = foldl (Data.Map.unionWith (+)) Data.Map.empty sleepMap
    in optimum minutesSlept


mostSleepyGuard minute sleepMap =
    let minuteSleepMap = Data.Map.map (Data.Map.findWithDefault 0 minute) sleepMap
    in optimum minuteSleepMap
...
main = do
    ...
    let events = sort $ map parseLine linesAsStrings
        ...
        minute = mostSleepyMinute sleepMap
        guard = mostSleepyGuard minute sleepMap
    print $ guard * minute
