...
simulate tracks [cart] [] =
    let Cart position _ _ = cart
    in position
...
simulate tracks carts (cart : queue) =
    ...
    in if any (collides newCart) (carts ++ queue)
       then let newCarts = filter (not . (collides newCart)) carts
                newQueue = filter (not . (collides newCart)) queue
            in simulate tracks newCarts newQueue
