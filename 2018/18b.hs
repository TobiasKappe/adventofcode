...
findCycle scan =
    let tortoises = iterate (tick) scan
        hares = iterate (tick . tick) scan
        Just (meet, _) = L.find (uncurry (==)) $ tail (zip tortoises hares)
        newHares = iterate (tick) meet
        Just mu = L.findIndex (uncurry (==)) (zip tortoises newHares)
        (muVal : remainder) = L.drop (mu+1) tortoises
        Just lam = L.findIndex (== muVal) remainder
    in (mu, lam+1)
...
main = do
    ...
    let scan = parseLines lines
        (mu, lam) = findCycle scan
        jump = 1000000000 `mod` lam
        final = iterate (tick) scan !! (jump+mu)
    print $ resourceValue final
