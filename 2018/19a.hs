...
simulate bind instructions = 
    simulate' 0 M.empty
    where simulate' ip registers =
              case M.lookup ip instructions of
                  Nothing -> registers
                  Just (opcode, args) -> 
                      let effect = operations M.! opcode
                          preRegisters = M.insert bind ip registers
...
main = do
    ...
    let bind = parseBind bindLine
        ...
        newRegisters = simulate bind instructions
    print $ getRegister newRegisters 0
